...
![Live evidence dashboard captured the moment the EV started charging from solar surplus](screenshots/evidence-dashboard.png)

There is a moment when a solar installation stops feeling like a set of devices and starts feeling like an operating system.

For me it was a quiet afternoon when the house decided to charge the Rivian.
...
![Cross-vendor data fusion: three vendors land in one HA state graph, and one new metric (compute-only power) emerges from the difference](screenshots/data-fusion-sankey.png)
